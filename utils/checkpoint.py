...
def save_checkpoint_iter(path_to_job, model, optimizer, epoch, gs, it, cfg):
    """
    Save a checkpoint.
    Args:
        model (model): model to save the weight to the checkpoint.
    ...
    # Write the checkpoint.
    name = "checkpoint_epoch_{:05d}:{:04d}.pyth".format(epoch, it)
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if "global_step" in checkpoint.keys():
            gs = checkpoint["global_step"]
        else:
            gs = -1
        return gs, epoch


def load_part_of_checkpoint(
    path_to_checkpoint,
    model,
    data_parallel=True,
    optimizer=None
    ...
    if "model_state" in checkpoint.keys():
        ...
        pretrained_state = {k:v for k,v in checkpoint["model_state"].items() if k in model_state}
        print("Load following parameters from the checkpoint: " + path_to_checkpoint + '\n', pretrained_state.keys())
        model_state.update(pretrained_state)
        ms.load_state_dict(model_state)
    else:
        ms.load_state_dict(checkpoint)
    # Load the optimizer state (commonly not done when fine-tuning)
    if optimizer:
